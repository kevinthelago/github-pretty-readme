...
const BAR_MAX_W = W - BAR_X - PAD - 52;
const BAR_H = 10;
const BAR_GAP = 52;
const BARS_TOP = 60;
...
const renderMonkeytypeChart = (modes) => {
    ...
    const leftPanel = `
    <text x="${CX}" y="46" text-anchor="middle" style="fill:var(--fg60)" font-size="11" letter-spacing="3" font-family="Arial, sans-serif">TYPING SPEED</text>
    <text x="${CX}" y="118" text-anchor="middle" style="fill:var(--fg)" font-size="52" font-weight="bold" font-family="Arial, sans-serif">${bestWpm}</text>
    <text x="${CX}" y="140" text-anchor="middle" style="fill:var(--fg35)" font-size="11" font-family="Arial, sans-serif">words per minute</text>
    <line x1="${CX - 28}" y1="${SEPARATOR_Y}" x2="${CX + 28}" y2="${SEPARATOR_Y}" style="stroke:var(--fg08)" stroke-width="1"/>
    <text x="${CX}" y="215" text-anchor="middle" style="fill:var(--fg25)" font-size="10" letter-spacing="2" font-family="Arial, sans-serif">MONKEYTYPE</text>`;
    ...
    const bars = modes.map(({ duration, wpm, acc }, i) => {
        ...
        return `
    <text x="${BAR_X}" y="${y - 4}" style="fill:var(--fg60)" font-size="10" letter-spacing="1" font-family="Arial, sans-serif">${duration}S</text>
    ...
    <text x="${BAR_X}" y="${y + BAR_H + 13}" style="fill:var(--fg30)" font-size="9" font-family="Arial, sans-serif">${acc.toFixed(1)}% acc</text>`;
    }).join('');
