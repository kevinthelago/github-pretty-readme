 ...
 * Returns an SVG fragment (no <svg> wrapper) so it can be embedded in any SVG.
 * Color references use CSS variables defined by THEME_CSS in the parent <svg>.
 *
...
const renderSpiderCore = (series, title, layout) => {
    ...
    const n = axes.length;
    if (n < 3) return `<text x="${cx}" y="${cy}" text-anchor="middle" style="fill:var(--fg40)" font-size="11" font-family="Arial, sans-serif">Not enough data</text>`;
    ...
    const gridRings = [0.25, 0.5, 0.75, 1.0].map(frac => {
        ...
        const opacity = frac === 1 ? 'var(--fg15)' : 'var(--fg07)';
        return `<polygon points="${pts}" fill="none" style="stroke:${opacity}" stroke-width="1"/>`;
    }).join('\n');

    const spokes = axes.map((_, i) => {
        const a = axisAngle(i);
        return `<line x1="${cx}" y1="${cy}" x2="${(cx + R * Math.cos(a)).toFixed(1)}" y2="${(cy + R * Math.sin(a)).toFixed(1)}" style="stroke:var(--fg10)" stroke-width="1"/>`;
    }).join('\n');
    ...
    const polygons = series.map(s => {
        ...
        const dots = axes.map((_, i) => {
            ...
            const a = axisAngle(i);
            return `<circle cx="${(cx + r * Math.cos(a)).toFixed(1)}" cy="${(cy + r * Math.sin(a)).toFixed(1)}" r="3" fill="${s.color}" style="stroke:var(--bg)" stroke-width="1.5"/>`;
        }).join('');
...
<text x="${nx}" y="${ny}" text-anchor="${anchor}" dominant-baseline="${baseline}" style="fill:var(--fg75)" font-size="${labelSize}" font-family="Arial, sans-serif">${clamp(tech.name, 12)}</text>`;
    }).join('\n');
...
<text x="${(lx + 16).toFixed(1)}" y="${legendY}" dominant-baseline="middle" style="fill:var(--fg60)" font-size="${labelSize}" font-family="Arial, sans-serif">${s.label}</text>`;
    }).join('\n');

    const titleUnderline = `<line x1="${titleX - 60}" y1="${titleY + 10}" x2="${titleX + 60}" y2="${titleY + 10}" style="stroke:var(--fg10)" stroke-width="1"/>`;

    return `<text x="${titleX}" y="${titleY}" text-anchor="middle" style="fill:var(--fg85)" font-size="${titleSize}" font-weight="bold" letter-spacing="3" font-family="Arial, sans-serif">${title.toUpperCase()}</text>
${titleUnderline}
${gridRings}
${spokes}
${polygons}
